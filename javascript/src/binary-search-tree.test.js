...
it("raises when node data is empty", () => {
  expect(() => makeNodeDF([])).toThrowError(/must/);
});

it("makes single node data", () => {
  const { data, left, right } = makeNodeDF([1]);
  expect(data).toBe(1);
